...
void MSTDataSource::setConnectionName(const QString& connectionName) {
...
void MSTDataSource::setFavorite(const QString& tag, const QDateTime& timeFromLog) {
	...
	for (auto& file : fileName) {
		if (QFile(file).fileTime(QFileDevice::FileModificationTime) <= timeFromLog) {
			continue;
		}
		TagLib::FileRef f;
...
QStringList MSTDataSource::addMusic(const QStringList& files) {
	...
	for (const QString& file : files) {
		...
		prepareStatement(
			"INSERT INTO musicInfo (title, artist, album, genre, year, track, fileName) VALUES (:title, :artist, :album, :genre, :year, :track, :fileName)");
...
QList<QueryItem> MSTDataSource::getMusicToTable(const short pageNum, const SortByEnum sortBy, const OrderByEnum orderBy) {
