...
void MusicSyncTool::getMusicConcurrent(const PathType path, const unsigned short page) {
	...
	QList<QueryItem> items = ds.getMusicToTable(page, static_cast<SortByEnum>(entity.sortBy),
	                                            static_cast<OrderByEnum>(entity.orderBy));
...
void MusicSyncTool::setFavorite(const QStringList& list, const PathType path, const TagLib::String& key,
	...
	if (entity.favoriteTag != "") {
		for (int i = 0; i < list.size(); i++) {
			ds.setFavorite(list.at(i), dateTime);
			emit current(i);
		}
	}
}

void MusicSyncTool::setRuleHit(const QStringList& fileList, const PathType path, const QList<LyricIgnoreRule>& rules,
                               const QDateTime& dateTime) {
	MSTDataSource& ds = path == PathType::LOCAL ? local : remote;
...
bool MusicSyncTool::isFormatSupported(const QString& fileName) const {
	return std::any_of(supportedFormat.begin(), supportedFormat.end(),
	                   [&fileName](const QString& format) {
